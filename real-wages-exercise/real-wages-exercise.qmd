...
## Parliamentary Report on Female Employment

*Select Committee on the Employment of Women and Children, 1851.
Figures represent the percentage of women of working age (15--60)
...
kable(df, booktabs = TRUE, align = rep("r", 5),
      caption = "Female Labour Force Participation (\\% of women aged 15--60 in paid employment)") |>
  kable_styling(latex_options = c("HOLD_position", "striped"), font_size = 10)
```
...
# --- Absolute cost of living: basket anchored to 1770 nominal wage ---
P_abs <- W_abs[1] * P_index

# --- Summary table ---
results <- data.frame(
  Year                          = dates,
  `Nominal Wage (£/week)`       = round(W_abs, 4),
  `Nominal Wage Index`          = round(W_index, 3),
  `Cost of Basket (£/week)`     = round(P_abs, 4),
  `Cost of Living Index`        = round(P_index, 3),
  `Real Wage Index`             = round(RW_index, 3),
  check.names = FALSE
)

kable(results, booktabs = TRUE, align = rep("r", 6),
      caption = "Real Wage Index, England 1770--1850 (1770 = 1.00). \\textit{Nominal Wage} and \\textit{Cost of Basket} are levels in £/week; index columns have base year 1770 = 1.00.") |>
